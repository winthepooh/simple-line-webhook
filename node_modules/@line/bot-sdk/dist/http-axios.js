...
class HTTPClient {
    ...
    wrapError(err) {
        if (err.response) {
            ...
            const { message } = err;
            return new exceptions_1.HTTPError(message, {
                statusCode: status,
            ...
            return new exceptions_1.RequestError(message, { code, originalError: err });
        }
        else if (err.config) {
            // unknown, but from axios
            const { message } = err;
            return new exceptions_1.ReadError(message, { originalError: err });
        }
