...
function middleware(config) {
    ...
    const _middleware = async (req, res, next) => {
        ...
        const body = await (async () => {
            if (isValidBody(req.rawBody)) {
                ...
                if (isValidBody(rawBody)) {
                    ...
                    throw new exceptions_1.JSONParseError("Invalid body", { raw: rawBody });
                }
            }
        })();
        if (!(0, validate_signature_1.default)(body, secret, signature)) {
            next(new exceptions_1.SignatureValidationFailed("signature validation failed", {
                signature,
            }));
            return;
        }
        ...
        try {
            ...
            const { message } = err;
            next(new exceptions_1.JSONParseError(message, { raw: strBody }));
        }
    };
    return (req, res, next) => {
